...
- FR と受け入れ条件の対応: FR-001〜FR-002a・FR-003〜FR-007（切替そのもの）は US1、FR-008〜FR-013（表示範囲）は US2 のシナリオ1〜5、FR-014〜FR-016（状態の保持）は US2 のシナリオ6〜8、FR-017〜FR-018（検証エラー）は US3 に対応。
- **2026-09-05 の `/speckit.clarify` セッション（2問）で対応言語を日本語・英語の2つに縮小し（要件 req-000015 を v1.1 へ更新済み）、言語選択肢の表記を各言語の自称で固定表示する方針（FR-002a）を追加した。** 再評価の結果、16項目の状態は変化せず全項目パスを維持。
